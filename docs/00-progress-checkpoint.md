...
- 新增字数统计工具：`backend/app/core/text_metrics.py`
- 优化正文生成：生成后自动检查实际字数，控制在目标字数约 ±10% 范围内。
- 本地演示模式下也会根据目标字数自动扩写或压缩，不再固定生成短模板。
- 前端文档信息区展示实际字数。
- 导出的 Word/txt 文件包含实际字数信息。
- 新增测试：`backend/tests/word_count_test.py`
- 全量测试通过，示例结果：500 字实际 489，800 字实际 774，1200 字实际 1124。
